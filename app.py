...
if 'results_fi' not in st.session_state: st.session_state.results_fi = None
...
@st.cache_data(ttl=3600)
def load_data_from_hf(start_year, hf_token, dataset_repo, universe_etfs=None):
    """Load data from HuggingFace dataset – simplified fallback for CSV."""
    if universe_etfs is None:
        universe_etfs = TARGET_ETFS
    try:
    ...
    if len(ret_cols) != len(universe_etfs):
        missing = [etf for etf in universe_etfs if not any(f'{etf}' in col for col in ret_cols)]
...
def run_tournament_engine(_features_df, _returns_df, rf_rate, tcost_bps, start_year, universe_etfs=None):
...
with st.sidebar:
    ...
    run_btn = st.button("🏦 Run FI Tournament", use_container_width=True)
    run_eq_btn = st.button("📈 Run Equity Tournament", use_container_width=True)

if run_btn:
    with st.status(f"Running FI Ensemble Tournament...") as status:
        try:
            ...
            for idx, start_year in enumerate(ensemble_years):
                ...
                data_tuple = load_data_from_hf(start_year, HF_TOKEN, HF_DATASET_REPO, tuple(TARGET_ETFS))
                ...
                result = run_tournament_engine(features_df, returns_df, rf, t_cost, start_year, tuple(TARGET_ETFS))
            ...
            status.update(label=f"FI Ensemble Tournament Complete!", state="complete")
        except Exception as e:
...
if run_eq_btn:
    with st.status(f"Running Equity Ensemble Tournament...") as status:
        try:
            rf = get_sofr_rate(FRED_API_KEY)
            ...
            for idx, start_year in enumerate(ensemble_years):
                ...
                data_tuple = load_data_from_hf(start_year, HF_TOKEN, HF_DATASET_REPO, tuple(EQ_ETFS))
                ...
                result = run_tournament_engine(features_df, returns_df, rf, t_cost, start_year, tuple(EQ_ETFS))
            ...
            status.update(label=f"Equity Ensemble Tournament Complete!", state="complete")
        except Exception as e:
            status.update(label=f"Error: {str(e)}", state="error")
            st.error(f"Failed to run tournament: {str(e)}")
            import traceback
    ...
    if st.session_state.results_fi:
        ...
        **Hold Period Optimization:** Each model tests 1-day, 3-day, and 5-day hold periods, selecting the one with the highest annualized return after transaction costs. The consensus hold period is the most common recommendation among periods that voted for the consensus ETF.
        ...
        st.info(
            ...
            f"**Universe:** {', '.join(TARGET_ETFS)}"
        )

with tab_eq:
    if st.session_state.results_eq:
        s = st.session_state.results_eq
